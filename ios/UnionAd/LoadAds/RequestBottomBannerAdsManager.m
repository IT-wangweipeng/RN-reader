//
//  RequestBottomBannerAdsManager.m
//  reader
//
...
@interface RequestBottomBannerAdsManager ()
<
...
@implementation RequestBottomBannerAdsManager
                            ...
                            error:(NSError *)error
{
  if (!error && unifiedNativeAdDataObjects.count > 0) {
    NSLog(@"成功请求到广告数据");
    if (self.delegate && [self.delegate respondsToSelector:@selector(bannerAdsLoadSuccess:)]) {
      [self.delegate bannerAdsLoadSuccess:unifiedNativeAdDataObjects];
    }
    return;
  }
  ...
  if (self.delegate && [self.delegate respondsToSelector:@selector(bannerAdsLoadFailed:)]) {
    [self.delegate bannerAdsLoadFailed:error];
  }
}
