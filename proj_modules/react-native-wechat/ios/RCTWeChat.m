...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shareToTimeLine:) name:@"shareToTimeLineNotification" object:nil];
      [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shareToWeChatFriend:) name:@"shareToWeChatFriendNotification" object:nil];
    }
    return self;
}
...
- (void)shareToTimeLine:(NSNotification *)noti {
  NSDictionary *data = noti.object;
  [self shareToWeixinWithData:data scene:WXSceneTimeline callback:^(NSArray *response) {
    
  }];
}

- (void)shareToWeChatFriend:(NSNotification *)noti {
  NSDictionary *data = noti.object;
  [self shareToWeixinWithData:data scene:WXSceneSession callback:^(NSArray *response) {
