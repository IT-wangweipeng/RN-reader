...
import * as serializeBookMaleAction from '../../../actions/serializeBookMaleAction'
...
class SerializeBookMalePage extends Component {
  ...
  componentWillMount() {
    this.props.actions.serializeBookMaleInit({
      sex: CONST_SEX,
    ...
    return (
      <View style={{flex: 1}}>
        ...
        <FlatList
          ...
          data={this.props.serializeBookMale.list}
          ...
          refreshControl={(
            <RefreshControl
              ...
              color="#ccc"
              refreshing={this.props.serializeBookMale.isRefreshing}
            />)}
  ...
  _loadMore = () => {
    if (!this.props.serializeBookMale.isLoadingMore && this.props.serializeBookMale.loadMore && !this.props.serializeBookMale.isRefreshing) {
      this.props.actions.serializeBookMaleLoadMore({
        sex: CONST_SEX,
        type: this.state.type,
        size: ONE_PAGE_SIZE,
        start: this.props.serializeBookMale.list.length,
      })
    }
  }
  ...
  _renderFooter = () => {
    const {serializeBookMale} = this.props
    if (serializeBookMale.list.length > 0) {
      return <ListFooter hasMore={serializeBookMale.loadMore}/>
    } else {
      return <View/>
    }
  }
...
const mapDispatchToProps = dispatch => ({
  actions: bindActionCreators({...serializeBookMaleAction,}, dispatch),
})

export default connect(({serializeBookMale, user}) => ({
  serializeBookMale,
  user,
}), mapDispatchToProps)(SerializeBookMalePage)
