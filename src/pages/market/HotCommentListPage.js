...
class HotCommentListPage extends Component {
  ...
  _handleLoadingMore = () => {
    ...
    const hasMore = bookComment.hotCommentsHasMore
    const start = bookComment.hotComments.length
  ...
  _ListFooterComponent = () => {
    const {bookComment, types} = this.props
    const comment = bookComment.hotComments
    const hasMore = bookComment.hotCommentsHasMore
    if (comment.length > 0) {
      return (
        <ListFooter
...
export default connect(({user, bookComment}) => ({
  user,
  bookComment
}), mapDispatchToProps)(HotCommentListPage)
